<template>
  <div>
    <div class="page-header">
      ...
      <button @click="openAddModal" v-if="authStore.isAdmin" class="btn-primary">Add New Grade</button>
    </div>

    <div class="table-container card">
      <div v-if="paginatedGrades.length" class="table-responsive">
        <table>
          <thead>
            <tr>
              ...
              <th @click="sortBy('grade')" class="sortable">
                Grade <span v-if="sortKey === 'grade'">{{ sortOrder === 'asc' ? '▲' : '▼' }}</span>
              </th>
      ...
      <p v-else class="no-data-message">No grades match your criteria.</p>
    </div>
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="saveGrade" class="modal-form">
          ...
          <div class="form-group">
            ...
            <select id="moduleId" v-model="currentGrade.module_id" required>
          ...
          <div class="form-group">
            ...
            <input id="assessmentName" type="text" v-model="currentGrade.assessment_name" required>
          </div>
          <div class="form-group">
            <label for="grade">Grade</label>
            <input id="grade" type="number" v-model="currentGrade.grade" required>
          </div>
...
import { getGrades, addGrade, updateGrade, deleteGrade, type Grade } from '@/api/gradeService'
...
// Sorting
const sortKey = ref<keyof Grade>('student_name')
...
const filteredAndSortedGrades = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    grades = grades.filter(g =>
      g.student_name?.toLowerCase().includes(lowerQuery) ||
      g.module_title?.toLowerCase().includes(lowerQuery) ||
      g.assessment_name.toLowerCase().includes(lowerQuery)
    )
  }

  // Filter by module
  if (selectedModule.value) {
    grades = grades.filter(g => g.module_title === selectedModule.value)
  }
...
const totalPages = computed(() => {
  return Math.ceil(filteredAndSortedGrades.value.length / itemsPerPage.value)
})
...
const paginatedGrades = computed(() => {
  ...
  return filteredAndSortedGrades.value.slice(start, end)
})
...
const saveGrade = async () => {
  ...
  messageType.value = ''
  try {
    if (isEditMode.value && currentGrade.value.id) {
      await updateGrade(currentGrade.value.id, currentGrade.value as Grade)
      message.value = 'Grade updated successfully!'
      messageType.value = 'success'
    } else {
      const { id, student_name, module_title, ...newGrade } = currentGrade.value;
      await addGrade(newGrade as Grade)
    ...
    message.value = error.response?.data?.message || 'Failed to save grade.'
    messageType.value = 'error'
  }
}

const deleteGrade = async (id: number) => {
  if (confirm('Are you sure you want to delete this grade? (Logical Delete)')) {
    try {
      await deleteGrade(id)
      message.value = 'Grade deleted successfully!'
      messageType.value = 'success'
      await fetchGrades()
    } catch (error: any) {
      console.error('Failed to delete grade:', error)
      message.value = error.response?.data?.message || 'Failed to delete grade.'
      messageType.value = 'error'
    }
  }
}
...
.modal-content {
  width: 90%;
  max-width: 600px; /* Adjusted width for Grade modal */
  background-color: var(--color-background);
  padding: 2rem 2.5rem;
  border-radius: 12px;
...
.modal-form .form-grid {
  display: grid;
  grid-template-columns: 1fr; /* Single column for grade form */
  gap: 1.5rem;
}
